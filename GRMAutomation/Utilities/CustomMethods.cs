...
namespace GRMAutomation.Utilities
{
    public static class CustomMethods
    {
        ...
        public static void SelectADropDownValue(this IWebDriver driver, IList<IWebElement> elements, string dropDownValue)
        {
            for (int i = 0; i < elements.Count; i++)
            {
                if (elements[i].Text.Equals(dropDownValue))
                {
                    elements[i].ClickOn(driver, 60);
                    break;
        ...
        public static string GetOTP(string MailId, string Password, string companyName, string clientName)
        {
            Thread.Sleep(20000);
            ...
            else if (MailId.Contains("inncrewin"))
            {
            ...
            var query = SearchQuery.DeliveredOn(DateTime.Parse(today))
                .And(SearchQuery.SubjectContains("Code for " + clientName))
                .And(SearchQuery.FromContains(companyName));

            string searchedMailSubject = "";
            string searchedMailBody = "";
            foreach (var uid in inbox.Search(query).Reverse())
            {
                var mail = inbox.GetMessage(uid);
                searchedMailSubject = "[match] " + uid + ": " + mail.Subject;
                searchedMailBody = mail.Body.ToString();
                break;
            }
            client.Disconnect(true);

            return searchedMailBody.Split(new[] { "code is " }, StringSplitOptions.None)[1].Substring(0, 8);
        }

        public static string GetTaxpayerLink(string MailId, string Password, string companyName)
        {
            var client = new ImapClient();
            if (MailId.Contains("gmail"))
            {
            ...
            var query = SearchQuery.DeliveredOn(DateTime.Parse(today))
               .And(SearchQuery.SubjectContains("Please review your tax return and sign your e-file forms"))
               .And(SearchQuery.FromContains(companyName));
            ...
            string link = searchedMailBody.Split(new[] { "Please select the below link to review your tax return" }, StringSplitOptions.None)[1];
            link = link.Replace("\n", "").Replace("\r", "").Replace("=", "").Split(new[] { "https://safesendreturns" }, StringSplitOptions.None)[1];
            link = "https://safesendreturns" + link;

            return link.Substring(0, 110);
        }
